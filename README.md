...
* `aws_mount_path` (string: `"aws"`) - The mount path of your Vault server's AWS secrets engine. This field is optional. If omitted, it will default to `"aws"`. This field is optional and will only be used when using the `iam` or `ec2` authentication method.
