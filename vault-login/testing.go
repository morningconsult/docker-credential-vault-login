...
// makeMockVaultServer creates a mock Vault server with two API endpoints - 
// /v1/auth/aws/login and /v1/<secret_path>. The behavior of these endpoints
// are configured via the testVaultServerOptions parameter. The login endpoint
// will only return 200 when the JSON payload of an HTTP request for this 
...
func makeMockVaultServer(t *testing.T, opts *testVaultServerOptions) *http.Server {
        ...
        mux.HandleFunc("/v1/auth/aws/login", awsAuthHandler(t, opts.role, opts.port))
        mux.HandleFunc(path.Join("/v1", opts.secretPath), dockerSecretHandler(t, opts.secret, opts.port))
        server := &http.Server{
